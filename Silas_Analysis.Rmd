...
df_time_rel <- df_time_rel[, c(2,3,4,5)]
...
ggplot(data=df_time_rel, aes(x=Datum, y=relative_change)) +
  geom_line(aes(group = Flughafen_Airline, colour = Flughafen_Airline )) +
...
df_time_rel2 <- df_time_rel %>% group_by(Datum) %>% summarise(mean_rel = mean(relative_change))
# Plot 1
ggplot() +
  geom_line(data = df_time_rel2, aes(x = Datum, y = mean_rel * 100)) +
...
# Plot Box_Plot
ggplot(df_time_rel, aes(Flughafen_Airline, mean_Preis)) +
...
df7 <- df %>% 
  ...
  select(Preis, Datenzugriff, Datum, Zeit) %>% 
  unite(DatumZeit, c("Datum", "Zeit"), sep = "")
